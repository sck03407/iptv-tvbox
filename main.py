...
class UpdateSource:
    ...
    async def visit_page(self, channel_names: list[str] = None):
        ...
        tasks_config = [
            ...
            ("subscribe", self._fetch_subscribe, "subscribe_result"),
            ("online_search", get_channels_by_online_search, "online_search_result"),
            ("epg", self._fetch_epg, "epg_result"),
        ]
        ...
        for setting, task_func, result_attr in tasks_config:
            if config.open_method.get(setting):
                if setting == "subscribe" or setting == "epg":
        ...
        if created_tasks:
            ...
            for (result_attr, setting), res in zip(task_bindings, results):
                if isinstance(res, Exception):
                    print(t("msg.error_name_info").format(name=setting, info=res))
