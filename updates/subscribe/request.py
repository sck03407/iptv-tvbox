...
async def get_channels_by_subscribe_urls(
    ...
    desc_text = pbar_desc
    if multicast:
        desc_text = f"Processing subscribe for multicast"
    elif hotel:
        desc_text = f"Processing subscribe for hotel"
    ...
    def process_subscribe_channels(subscribe_info: str | dict) -> defaultdict:
        ...
        try:
            ...
            if callback:
                callback(
                     f"{t('msg.task_remaining').format(mode=mode_name, remaining=subscribe_urls_len - pbar.n, time=get_pbar_remaining(n=pbar.n, total=pbar.total, start_time=start_time))}",
