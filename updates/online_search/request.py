...
async def get_channels_by_online_search(names, callback=None):
    ...
    def process_channel_by_online_search(name):
        ...
        try:
            ...
            if callback:
                callback(
                    t("msg.task_online_search_remaining").format(remaining=names_len - pbar.n, time=get_pbar_remaining(n=pbar.n, total=pbar.total, start_time=start_time)),
    ...
    if callback:
        callback(t("msg.task_online_search_total").format(total=names_len), 0)
