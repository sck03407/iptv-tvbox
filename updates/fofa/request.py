...
async def get_channels_by_fofa(urls=None, multicast=False, callback=None):
    ...
    if config.open_request:
        ...
        if callback:
            callback(
                t("msg.task_fofa_total").format(mode=mode_name, total=fofa_urls_len),
                0,
            )
        ...
        def process_fofa_channels(fofa_info):
            ...
            try:
                ...
                if callback:
                    callback(
                        t("msg.task_fofa_remaining").format(mode=mode_name, remaining=remain, time=get_pbar_remaining(n=pbar.n, total=pbar.total, start_time=start_time)),
        ...
        if callback:
            callback(
                t("msg.task_fofa_completed").format(mode=mode_name),
                100,
            )
        if not open_driver:
            close_session()
