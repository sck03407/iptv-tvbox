...
def get_multicast_region_result_by_rtp_txt(callback=None):
        ...
        rtp_file_list = [
            ...
            if filename.endswith(".txt")
               ...
               and (
                       ...
                       or config.multicast_region_list == ["全部"]
               )
        ]

    total_files = len(rtp_file_list)
    if callback and total_files > 0:
        callback(t("msg.loading_local_multicast_data").format(total=total_files), 0)
    ...
    for filename in rtp_file_list:
        region, _, type_ext = filename.partition("_")
        type = type_ext.rsplit(".", 1)[0]
        with open(
                os.path.join(rtp_path, filename), "r", encoding="utf-8"
        ) as f:
            for line in f:
                try:
                    ...
                    if "," in line_strip:
                        ...
                        if url and "rtp://" in url:
                             channel_name = format_channel_name(name)
                             multicast_result[channel_name][region][type].append(url.strip())
                except:
                    continue
        pbar.update()
        if callback:
            ...
            callback(
                t("msg.loading_multicast_region_data").format(region=region, type=type, remaining=remaining_files, time=estimated_time),
                int((pbar.n / total_files) * 100) if total_files > 0 else 100,
            )
