    ...
    if config.open_request:
        ...
        def process_channel_by_multicast(region, type):
            ...
            try:
                ...
                if callback:
                    callback(
                        t("msg.task_multicast_foodie_remaining").format(remaining=region_type_list_len - pbar.n, time=get_pbar_remaining(n=pbar.n, total=pbar.total, start_time=start_time)),
        ...
        if config.open_multicast_foodie:
            ...
            if callback:
                callback(
                    t("msg.task_multicast_foodie").format(channel=len(names), region=region_type_list_len),
                    0,
                )
            start_time = time()
